...
import { ButtonDown, ButtonUp, ContainerProject26, Information, Landscape } from "./styles";
...
export function Project26(){
  ...
  function handleLandscapeSelection(buttonClicked:ButtonClicked):void{
    if(buttonClicked==='up'){
      setLandscapeSelected(state=>state < landscapes.length - 1 ? state + 1 : 0)
    }
    else if(buttonClicked==='down'){
      setLandscapeSelected(state=>state === 0 ? landscapes.length - 1 : state -1 )
    }
  }
  ...
  return (
    <ContainerProject26>
      ...
      {
        landscapes.map((landscape,index)=>(
          <Landscape backgroundImage={landscape.img}  y={index === landscapeSelected ? 0 : index - landscapeSelected}/>
        ))
      }
